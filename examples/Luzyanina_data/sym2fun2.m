...
function [fun] = sym2fun2(varargin)

%% CHECK AND ASSIGN INPUTS
if nargin >= 2
    expr = varargin{1};
    % Number of variable vectors
    if strcmp(varargin{end-1},'name')
        nvar = nargin-3;
        exp_var_name = varargin{end};
    else
        nvar = nargin-1;
        for i = 1:nvar
            exp_var_name{i} = ['theta' num2str(i,'%d')];
        end
    end
else
...
for k=1:nvar
    ...
    if (size(argsym,1) == 1) && (size(argsym,2) == 1)
        if nvar == 1
            x{k} = sym([exp_var]);
        else
            x{k} = sym([exp_var num2str(k)]);
        end
		...
		for k=1:nvar
            for i=size(x{k},1):-1:1
                for j=size(x{k},2):-1:1
                    if nvar == 1
                        if (size(x{k},1) == 1) && size(x{k},2) == 1
                                next = strrep(next,char(x{k}),['??']);                            
                        else
                            if size(x{k},2) == 1
...
exprstr = strrep(exprstr,'?',argstr);

end
% Exhange variable names
for k=1:nvar
    if nvar == 1
        exprstr = strrep(exprstr,exp_var,exp_var_name{1});
    else
        exprstr = strrep(exprstr,[exp_var num2str(k)],exp_var_name{k});
    end
end
